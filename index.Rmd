...
housingdatalabels <- sprintf(
  ...
  "<strong># of Available Total Units: </strong>%g<br/>"),
  ...
  housingdata$Total.Units
  ) %>% 
  lapply(HTML)

getColor <- function(housingdata) {
  sapply(housingdata$Total.Units, function(Total.Units) {
  if(Total.Units >= 100) {
    "green"
  } else {
    "red"
...
map <- leaflet() %>% setView(lng = -74.0060, lat = 40.7128, zoom = 11) %>% 
  ...
  addCircleMarkers(housingdata$Longitude, 
             ...
             color = getColor(housingdata),
             radius = ifelse(housingdata$Total.Units >= 100, 10, 5),
             stroke = FALSE,
             fillOpacity = .5,
  ...
  addLegend(
            ...
            group="Income") %>% 
  addLayersControl(
    overlayGroups = c("Income", "Total Affordable Housing Units Available by Property"),
